...
class BookCheck: 
	def __init__(self,root):
		...
		self.v = tk.IntVar()
		self.check = tk.Checkbutton(frame,text='存档',variable=self.v).grid(row=1, column=1, sticky=W, padx=5, pady=5)
	...
	def infoGet(self,Keyinfo):
		...
		try:
			re_author_intro=re.compile(r'<db:attribute name="author-intro">([\s\S]*)</db:attribute>')
		...
		try:
			re_summary=re.compile(r'<summary>([\s\S]*)</summary>')
			summary=str(re.findall(re_summary,response)[0])
		...
		if self.v.get()==1:
			...
			if dataYN:
				with open('BookData.csv', 'a', encoding='gb18030', newline='') as csvfile:
